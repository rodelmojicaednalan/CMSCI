 ...
 * Sidebarwidgetmodel
 *
 * @package Sidebarwidgetmodel
 */

class Sidebarwidgetmodel extends CI_Model
{
	//the table associated with the user model
	private $table_name = 'sidebar_widget';
	private $primary_key = 'sidebar_widget_id';
	...
	function get($options = array())
	{
		...
		$this->db->join('widget_type', 'sidebar_widget.widget_type_id = widget_type.widget_type_id', 'LEFT');
		// Qualification
		if(isset($options['widget_type_id'])){
			$this->db->where('widget_type.widget_type_id', $options['widget_type_id']);
		}

		if(isset($options['keyword'])){
			$this->db->where("sidebar_widget.sidebar_widget_name like '%".$this->db->escape_like_str($options['keyword'])."%'");
		}

		if(isset($options['id'])){
			$this->db->where('sidebar_widget.sidebar_widget_id', $options['id']);
		}
