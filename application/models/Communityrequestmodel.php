 ...
 *
 * @package Communityrequestmodel
 */

class Communityrequestmodel extends CI_Model
{

    //the table associated with the group model
	private $table_name = 'community_request';
	
	private $primary_key = 'community_request_id';
	//the required column/values for the community_request tbl
	private $required_columns = array();

	//the default column/values for the community_request tbl
	...
	function get($options = array())
	{
        ...
        if(isset($options['search_value']))
        	$this->db->like('community_request.community_request_name', $options['search_value']);
		
		// filter status
		if(isset($options['filter_status']))
			$this->db->where('community_request.community_request_status', $options['filter_status']);
