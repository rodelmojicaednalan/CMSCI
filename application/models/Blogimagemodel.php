 ...
 * Blogimagemodel
 *
 * @package Blogimagemodel
 */

class Blogimagemodel extends CI_Model
{

    //the table associated with the blog_images model
	private $table_name = 'blog_images';
	
	private $primary_key = 'blog_image_id';
	//the required column/values for the blog_images
	private $required_columns = array();

	//the default column/values for the blog_images
	...
	{
		$this->db->join('file_upload', 'file_upload.file_upload_id = blog_images.file_upload_id');

		if(isset($options['blog_id']))
			$this->db->where('blog_images.blog_id', $options['blog_id']);
		
		if(isset($options['order']))
			$this->db->where('blog_images.order', $options['order']);
		// limit / offset
